...
int main(int argc, char *argv[]){
    ...
    // setup the state nodes

    for ( int i =0; i < 2; i++ ){
        for ( int j = 0; j < 3; j++){
            i1.state[i][j] = initState[i][j];
            g1.state[i][j] = goalState[i][j];
            f1.state[i][j] = initState[i][j];
        }
    }
    ...
    if ( argv[3] == modes[0] ) { 
        struct node *goal = bfs(i1, g1, &f1, argv, out1);
    }
